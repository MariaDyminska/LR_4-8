...
//                        return;
//                    case "99": // спеціальний пункт для тесту
//                        logger.info("Тестовий виклик критичної помилки");
//                        throw new RuntimeException("Тестова критична помилка для відправки e-mail!");
//
//                    default:
...
// ІМПОРТУЙТЕ ВАШ СЕРВІС ПОШТИ ТУТ (розкоментуйте, коли підключите клас):
    ...
    public void show() {
        while (true) {
            ...
            try {
                ...
                // 1. Логуємо помилку у файл/консоль
                logger.error("Помилка при обробці дії користувача", e);
                System.out.println("Помилка: " + e.getMessage());

                // 2. ВІДПРАВЛЯЄМО ЛИСТ (ДОДАНО)
                System.out.println(">>> [SYSTEM] Спроба відправки звіту на E-mail...");

                try {
                    // !!! ТУТ МАЄ БУТИ ВИКЛИК ВАШОГО КЛАСУ ВІДПРАВКИ !!!
                    // Приклад:
                    // EmailService.send("admin@bank.com", "Критична помилка в меню", e.toString());

                    // Або виклик статичного методу, якщо він у вас є:
                    // main.Main.sendErrorEmail(e.getMessage());

                    System.out.println(">>> [SYSTEM] Запит на відправку пошти сформовано.");
                } catch (Exception mailError) {
                    System.err.println("Не вдалося відправити лист: " + mailError.getMessage());
                }
            }
        }
    }

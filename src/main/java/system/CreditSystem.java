...
//
//    public List<Credit> getAllCredits(){ return creditList; }
//
//    public void addCredit(Credit c){ creditList.add(c); record(c.getId(), "Додано кредит: " + c.getName()); }
//    public void removeCredit(int id){ creditList.removeIf(c->c.getId()==id); record(id, "Видалено кредит"); }
//    public Credit findById(int id){ for(Credit c: creditList) if(c.getId()==id) return c; return null; }
//    public void updatePartial(int id, String name, String ssum, String sterm, String srate){
//        Credit c = findById(id); if(c==null) return;
...
//    }
//    public void listCredits(){ if(creditList.isEmpty()) System.out.println("Немає кредитів"); else creditList.forEach(System.out::println); }
//
//    public void save(String fname) throws Exception { DataFileManager.saveObject(this, fname); }
//    public void load(String fname) throws Exception {
//        Object o = DataFileManager.loadObject(fname);
//        if(o instanceof CreditSystem){ CreditSystem cs = (CreditSystem)o; this.creditList = cs.creditList; this.history = cs.history; }
//        else throw new Exception("Невірний файл");
//    }
//
//    // payments and operations
//    public void makePayment(int id, double amount){
//        Credit c = findById(id); if(c==null){ System.out.println("Кредит не знайдено"); return; }
//        c.setSum(c.getSum() - amount); record(id, "Платіж: " + amount);
//        if(c.getSum() <= 0){ c.setStatus(CreditStatus.CLOSED); record(id, "Кредит закрито"); System.out.println("Кредит закрито"); }
//        else System.out.println("Залишок: " + c.getSum());
//    }
//
//    public void earlyPayoff(int id, double amount){ makePayment(id, amount); record(id, "Дострокове погашення: " + amount); }
//
//    public void refinance(int id, double newSum, double newRate, int newTerm){
//        Credit c = findById(id); if(c==null){ System.out.println("Кредит не знайдено"); return; }
//        if(newSum>0) c.setSum(newSum); if(newRate>=0) c.setInterestRate(newRate); if(newTerm>0) c.setTermMonths(newTerm);
//        record(id, "Рефінансовано"); System.out.println("Оновлено: " + c);
//    }
//
//    // search and helpers
...
//        double bestScore = Double.MAX_VALUE; Credit best = null;
//        for(Credit c: creditList){
...
//        double bestR = Double.MAX_VALUE; Credit best = null;
//        for(Credit c: creditList){
...
//        Credit c = findById(id); if(c==null){ System.out.println("Кредит не знайдено"); return; }
//        double P = c.getSum(); int n = c.getTermMonths(); double monthly = (c.getInterestRate()/100.0)/12.0;
//        DecimalFormat df = new DecimalFormat("#.##");
//        System.out.println("Місяць | Платіж | Відсотки | Принципал | Залишок");
//        if(monthly<=0){
//            double pay = P / n; double rem = P;
//            for(int i=1;i<=n;i++){ double interest=0; double principal=pay; rem-=principal; System.out.printf("%d | %s | %s | %s | %s\n", i, df.format(pay), df.format(interest), df.format(principal), df.format(Math.max(rem,0.0))); }
//        } else {
//            double pay = (monthly * P) / (1 - Math.pow(1+monthly, -n)); double rem = P;
//            for(int i=1;i<=n;i++){ double interest = rem * monthly; double principal = pay - interest; rem -= principal; System.out.printf("%d | %s | %s | %s | %s\n", i, df.format(pay), df.format(interest), df.format(principal), df.format(Math.max(rem,0.0))); }
//        }
//    }
...
//
//    // history
//    private void record(int id, String event){ history.computeIfAbsent(id, k->new ArrayList<>()).add(event); }
//    public List<String> getHistory(int id){ return history.getOrDefault(id, new ArrayList<>()); }
//}
package system;
import model.*;
import java.util.*;
